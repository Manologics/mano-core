...
const FontStyle = () => (
  <style>{`
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .gold-glow-btn { animation: goldPulse 2.8s ease-in-out infinite; }
    .gold-glow-btn:hover {
      animation: none !important;
      box-shadow: 0 0 70px #f5c518cc, 0 0 140px #f5c51866 !important;
      transform: scale(1.04) !important;
    }
    .gold-glow-btn:focus { outline: none; }
    .red-loss { animation: redFlicker 3s ease-in-out infinite; }
    .gold-headline { animation: goldText 3s ease-in-out infinite; }
  `}</style>
);
...
// ─── SECTION DIVIDER ──────────────────────────────────────────────────────────
function Divider({ color = GOLD, dim = "18" }) {
  const c = color;
  return (
    <div style={{ display:"flex",alignItems:"center",gap:"14px",padding:"0 24px",maxWidth:"880px",margin:"0 auto" }}>
      <div style={{ flex:1,height:"1px",background:`linear-gradient(90deg,transparent,${c}${dim},transparent)` }}/>
      <div style={{ width:"5px",height:"5px",borderRadius:"50%",background:c,opacity:0.5,boxShadow:`0 0 10px ${c}` }}/>
      <div style={{ flex:1,height:"1px",background:`linear-gradient(90deg,transparent,${c}${dim},transparent)` }}/>
    </div>
  );
}
...
function ScoreBadge({ score }) {
  const map = { HOT:[REDB,"#cc202020"], WARM:[GOLD,"#f5c51820"], COLD:["#888","#88888820"] };
...
function CalculatorModal({ onClose }) {
  ...
  const INP = {
    ...
    fontFamily:"'DM Sans',sans-serif",fontSize:"15px",outline:"none",boxSizing:"border-box",
  };
  ...
  async function handleSubmit() {
    ...
    try {
      await fetch("https://mano-dd309130.base44.app/functions/landingLeadCapture", {
        ...
        body:JSON.stringify({ name,phone,missedCallsPerWeek:calls,jobValue,monthlyLoss:missedMonthly,source:"calculator" })
      });
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.96)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:"16px" }}
      onClick={e=>{ if(e.target===e.currentTarget) onClose(); }}>
      <div style={{ background:"#0e0e0e",border:`1px solid ${GOLD}35`,borderRadius:"18px",padding:"clamp(24px,5vw,40px) clamp(20px,5vw,36px)",maxWidth:"480px",width:"100%",position:"relative",boxShadow:`0 0 80px ${GOLD}14, 0 40px 80px rgba(0,0,0,0.9)` }}>
        ...
        {step===1 && <>
          ...
          <h2 style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:"clamp(26px,5vw,34px)",color:WHITE,margin:"0 0 6px",letterSpacing:"1px",lineHeight:1.1 }}>How much are you losing?</h2>
          <p style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"14px",color:MID,margin:"0 0 24px",lineHeight:1.6 }}>Enter your numbers and we'll calculate your exact monthly loss.</p>
          <div style={{ display:"flex",flexDirection:"column",gap:"14px",marginBottom:"28px" }}>
            <div>
              <label style={{ display:"block",fontFamily:"'Space Mono',monospace",fontSize:"9px",color:DIM,letterSpacing:"2px",marginBottom:"7px" }}>MISSED CALLS PER WEEK</label>
              <input type="number" min="1" placeholder="e.g. 5" value={calls} onChange={e=>setCalls(e.target.value)}
                style={{ ...INP,border:`1px solid ${calls?GOLD+"55":"#333"}`,color:GOLD,fontFamily:"'Bebas Neue',sans-serif",fontSize:"36px",textAlign:"center",padding:"20px 16px",boxShadow:calls?`inset 0 0 20px ${GOLD}08`:"none" }}/>
            </div>
            <div>
              <label style={{ display:"block",fontFamily:"'Space Mono',monospace",fontSize:"9px",color:DIM,letterSpacing:"2px",marginBottom:"7px" }}>AVERAGE JOB VALUE ($)</label>
              <input type="number" min="1" placeholder="500" value={jobValue} onChange={e=>setJobValue(e.target.value)}
                style={{ ...INP,textAlign:"center",fontSize:"18px",fontWeight:"600",color:LIGHT }}/>
            </div>
          </div>
          <button onClick={()=>canNext1&&setStep(2)} disabled={!canNext1} className={canNext1?"gold-glow-btn":""}
        ...
        {step===2 && <>
          <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"9px",color:REDB,letterSpacing:"3px",marginBottom:"16px" }}>STEP 2 OF 3 — YOUR MONTHLY LOSS</div>
          <div style={{ textAlign:"center",padding:"16px 0 24px" }}>
            <div style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"15px",color:MID,marginBottom:"10px" }}>You're losing an estimated</div>
            <div className="red-loss" style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:"clamp(64px,15vw,100px)",color:REDB,lineHeight:1,letterSpacing:"2px",marginBottom:"8px" }}>
              {fmt(missedMonthly)}
            </div>
            <div style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"16px",color:LIGHT,fontWeight:"600" }}>per month in missed calls.</div>
          </div>
          <div style={{ background:`linear-gradient(135deg,${RED}18,#080404)`,border:`1px solid ${RED}44`,borderRadius:"10px",padding:"16px 18px",marginBottom:"28px" }}>
            <div style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"14px",color:LIGHT,fontWeight:"600",lineHeight:1.6 }}>
              That's <strong style={{ color:REDB }}>{fmt(missedMonthly*12)}/year</strong> going straight to competitors who answer their phone.
            </div>
          </div>
          <button onClick={()=>setStep(3)} className="gold-glow-btn"
        ...
        {step===3 && !submitted && <>
          ...
          <h2 style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:"clamp(24px,5vw,32px)",color:WHITE,margin:"0 0 6px",letterSpacing:"1px",lineHeight:1.1 }}>Where should we send your plan?</h2>
          <p style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"14px",color:MID,margin:"0 0 22px",lineHeight:1.6 }}>We'll reach out to show you how Mano recovers your <strong style={{ color:REDB }}>{fmt(missedMonthly)}/mo</strong> in missed jobs.</p>
          ...
          <button onClick={handleSubmit} disabled={!name||!phone||saving} className={name&&phone&&!saving?"gold-glow-btn":""}
          ...
          <div style={{ marginTop:"10px",fontFamily:"'Space Mono',monospace",fontSize:"9px",color:DIM,textAlign:"center",letterSpacing:"1px" }}>No spam. We'll text you within minutes.</div>
          <button onClick={()=>setStep(2)} style={{ width:"100%",marginTop:"8px",padding:"8px",background:"transparent",border:"none",color:"#555",fontFamily:"'DM Sans',sans-serif",fontSize:"13px",cursor:"pointer" }}>← Back</button>
        </>}
        ...
        {submitted && <>
          <div style={{ textAlign:"center",padding:"20px 0" }}>
            ...
            <p style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"15px",color:MID,margin:"0 0 28px",lineHeight:1.7 }}>
              ...
              <strong style={{ color:REDB }}>{fmt(missedMonthly)}/month</strong> in missed calls.
            </p>
            <button onClick={()=>{window.open(CALENDLY_URL,"_blank");onClose();}} className="gold-glow-btn"
        ...
        <button onClick={onClose}
          style={{ position:"absolute",top:"16px",right:"20px",background:"none",border:"none",color:DIM,fontSize:"22px",cursor:"pointer",lineHeight:1,transition:"color 0.2s" }}
          onMouseEnter={e=>e.currentTarget.style.color=WHITE} onMouseLeave={e=>e.currentTarget.style.color=DIM}>✕</button>
      </div>
    </div>
  );
}
    ...
    <div style={{ background:"#0d0d0d",border:"1px solid #222",borderRadius:"16px",overflow:"hidden" }}>
      <div style={{ background:"#111",borderBottom:"1px solid #222",padding:"14px 18px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
        ...
        <div style={{ textAlign:"right" }}>
          <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"8px",color:"#555",letterSpacing:"2px" }}>LIVE SIMULATION</div>
          <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"8px",color:"#555",letterSpacing:"1px" }}>HVAC SCENARIO</div>
        </div>
      </div>
      <div style={{ height:"360px",overflowY:"auto",padding:"20px",display:"flex",flexDirection:"column",gap:"14px",background:"#080808" }}>
        {visible.length===0&&!running&&(
          <div style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"14px" }}>
            ...
            <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"10px",color:"#444",textAlign:"center",letterSpacing:"1px",lineHeight:2 }}>
        ...
        {visible.map((m,i)=>(
          <div key={i} style={{ display:"flex",flexDirection:"column",alignItems:m.from==="ai"?"flex-start":"flex-end" }}>
            <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"9px",color:"#555",marginBottom:"4px",marginLeft:m.from==="ai"?"2px":0,marginRight:m.from!=="ai"?"2px":0 }}>
              {m.from==="ai"?"Mano":"Marcus"}
            </div>
            <div style={{ maxWidth:"78%",padding:"12px 16px",borderRadius:m.from==="ai"?"4px 16px 16px 16px":"16px 4px 16px 16px",background:m.from==="ai"?"#1a1a1a":`${GOLD}14`,border:m.from==="ai"?"1px solid #2a2a2a":`1px solid ${GOLD}44`,fontFamily:"'DM Sans',sans-serif",fontSize:"13px",color:m.from==="ai"?LIGHT:GOLD,lineHeight:1.65 }}>
              {m.text}
            </div>
          </div>
        ))}
        {running&&visible.length>0&&visible[visible.length-1].from==="lead"&&(
          <div style={{ display:"flex",flexDirection:"column",alignItems:"flex-start" }}>
            <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"9px",color:"#555",marginBottom:"4px",marginLeft:"2px" }}>Mano</div>
            <div style={{ padding:"12px 18px",borderRadius:"4px 16px 16px 16px",background:"#1a1a1a",border:"1px solid #2a2a2a" }}>
              <span style={{ fontSize:"20px",letterSpacing:"3px",color:"#555" }}>···</span>
            </div>
          </div>
        )}
        <div ref={bottomRef}/>
      ...
      <div style={{ background:"#111",borderTop:"1px solid #222",padding:"14px 18px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
        ...
        <button onClick={run} disabled={running} className={!running?"gold-glow-btn":""}
          style={{ padding:"10px 22px",borderRadius:"7px",fontFamily:"'Space Mono',monospace",fontSize:"11px",fontWeight:"700",letterSpacing:"1px",cursor:running?"not-allowed":"pointer",background:running?"#1a1a1a":`linear-gradient(135deg,${GOLD},${GOLDD})`,color:running?"#555":"#000",border:"none",transition:"background 0.2s" }}>
    ...
    <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:"12px" }}>
      {QUAL_STEPS.map((s,i)=>(
        <div key={i} onClick={()=>setActive(active===i?null:i)}
          style={{ background:active===i?"#141414":"#0d0d0d",border:`1px solid ${active===i?s.color+"66":"#2a2a2a"}`,borderRadius:"14px",padding:"22px 20px",cursor:"pointer",transition:"all 0.25s",boxShadow:active===i&&s.color===GOLD?`0 0 20px ${GOLD}20`:active===i&&s.color===REDB?`0 0 20px ${RED}20`:"none" }}>
          <div style={{ fontSize:"28px",marginBottom:"12px" }}>{s.icon}</div>
          <div style={{ display:"flex",alignItems:"center",gap:"8px",marginBottom:active===i?"10px":"0" }}>
            <div style={{ width:"20px",height:"20px",borderRadius:"50%",background:`${s.color}20`,border:`1px solid ${s.color}66`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
          ...
          {active!==i&&<div style={{ fontFamily:"'Space Mono',monospace",fontSize:"9px",color:"#555",marginTop:"6px",letterSpacing:"0.5px" }}>tap to expand</div>}
        </div>
      ))}
    </div>
  );
...
function Pipeline() {
  ...
  const stageColors = { New:"#888", Contacted:"#aaa", Qualified:GOLD, Booked:GOLD };
        ...
        return (
          <div key={stage}>
            <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"10px" }}>
              ...
              <div style={{ background:`${c}20`,border:`1px solid ${c}55`,borderRadius:"10px",padding:"1px 8px",fontFamily:"'Space Mono',monospace",fontSize:"9px",color:c }}>{stageLeads.length}</div>
            </div>
            <div style={{ display:"flex",flexDirection:"column",gap:"8px",minHeight:"60px" }}>
              {stageLeads.map(l=>(
                <div key={l.id} style={{ background:"#111",border:"1px solid #222",borderRadius:"10px",padding:"12px",opacity:moving===l.id?0.3:1,transition:"opacity 0.3s",boxShadow:stage==="Booked"?`0 0 12px ${GOLD}18`:"none" }}>
                  ...
                  <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"9px",color:"#555",marginBottom:"10px" }}>{l.time}</div>
                  {stage!=="Booked"&&(
                    <button onClick={()=>advance(l.id)}
                      style={{ width:"100%",padding:"6px",borderRadius:"5px",fontFamily:"'Space Mono',monospace",fontSize:"9px",fontWeight:"700",cursor:"pointer",background:"transparent",border:`1px solid ${stageColors[nextStage]}55`,color:stageColors[nextStage],letterSpacing:"0.5px",transition:"all 0.2s" }}>
...
function ROIPanel() {
  return (
    <div>
      <div style={{ background:`linear-gradient(135deg,${RED}18,#0a0808,${RED}0a)`,border:`1px solid ${RED}44`,borderRadius:"16px",padding:"40px 32px",textAlign:"center",marginBottom:"16px",boxShadow:`0 0 40px ${RED}14` }}>
        <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"10px",color:MID,letterSpacing:"3px",marginBottom:"14px" }}>IF YOU MISS JUST 5 CALLS A WEEK</div>
        ...
        <div style={{ display:"inline-block",background:`${GOLD}12`,border:`1px solid ${GOLD}44`,borderRadius:"8px",padding:"12px 24px",boxShadow:`0 0 20px ${GOLD}18` }}>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:"10px" }}>
        {ROI_ITEMS.map(r=>(
          <div key={r.label} style={{ background:"#0f0f0f",border:`1px solid ${GOLD}22`,borderRadius:"12px",padding:"20px",boxShadow:`0 0 16px ${GOLD}08` }}>
            <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"9px",color:DIM,letterSpacing:"1px",marginBottom:"10px" }}>{r.label}</div>
            <div style={{ display:"flex",alignItems:"baseline",gap:"12px" }}>
              <span style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"13px",color:REDB,textDecoration:"line-through",opacity:0.8 }}>{r.before}</span>
            ...
            <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"9px",color:"#444",marginTop:"4px",letterSpacing:"0.5px" }}>with Monkee Bizz AI</div>
          </div>
        ))}
      </div>
    </div>
...
export default function Demo() {
  ...
  function CTAPrimary({ style: extraStyle = {} }) {
    return (
      <button onClick={()=>setShowModal(true)} className="gold-glow-btn"
        style={{ background:`linear-gradient(135deg,${GOLD},${GOLDD})`,color:"#000",border:"none",padding:"17px 36px",borderRadius:"9px",fontFamily:"'Space Mono',monospace",fontSize:"12px",fontWeight:"700",cursor:"pointer",letterSpacing:"1px",transition:"transform 0.2s, box-shadow 0.2s",...extraStyle }}>
  ...
  function CTASecondary({ style: extraStyle = {} }) {
    return (
      <button onClick={()=>setShowModal(true)}
        style={{ background:"transparent",color:MID,border:"1px solid #444",padding:"16px 28px",borderRadius:"9px",fontFamily:"'DM Sans',sans-serif",fontSize:"14px",fontWeight:"600",cursor:"pointer",transition:"all 0.2s",...extraStyle }}
        onMouseEnter={e=>{ e.currentTarget.style.borderColor="#888"; e.currentTarget.style.color=WHITE; }}
        onMouseLeave={e=>{ e.currentTarget.style.borderColor="#444"; e.currentTarget.style.color=MID; }}>
        Book a Demo
      </button>
    );
  }
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#060606",color:LIGHT }}>
      ...
      {/* NAV */}
      <nav style={{ borderBottom:"1px solid #1a1a1a",padding:"18px 28px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,background:"rgba(6,6,6,0.97)",backdropFilter:"blur(14px)",zIndex:100 }}>
        <div>
      ...
      <section style={{ maxWidth:"880px",margin:"0 auto",padding:"92px 24px 64px",textAlign:"center" }}>
        <div style={{ display:"inline-block",background:`${RED}14`,border:`1px solid ${RED}55`,borderRadius:"20px",padding:"5px 18px",marginBottom:"28px",boxShadow:`0 0 16px ${RED}18` }}>
          <span style={{ fontFamily:"'Space Mono',monospace",fontSize:"9px",color:REDB,letterSpacing:"3px" }}>FOR HVAC CONTRACTORS</span>
        </div>
        <h1 style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:"clamp(52px,9vw,108px)",color:WHITE,lineHeight:0.95,margin:"0 0 24px",letterSpacing:"2px",textShadow:"0 0 60px rgba(255,255,255,0.06)" }}>
        ...
        <p style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"16px",color:REDB,maxWidth:"480px",margin:"0 auto 18px",lineHeight:1.6,fontWeight:"500",fontStyle:"italic" }}>
          While you're missing calls, your competitors are closing them.
        </p>
        <p style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"17px",color:MID,maxWidth:"500px",margin:"0 auto 16px",lineHeight:1.8 }}>
          Mano responds in seconds, qualifies the lead, and books the job — automatically.
        </p>
        <p style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"15px",color:DIM,maxWidth:"420px",margin:"0 auto 48px",lineHeight:1.6 }}>
        ...
        <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"10px",color:"#555",letterSpacing:"1px" }}>Free 30-min walkthrough. No pressure.</div>
      </section>

      {/* DIVIDER — hero → pain */}
      <div style={{ padding:"8px 0 40px" }}>
        <Divider color={RED} dim="20"/>
      </div>

      {/* PAIN SECTION */}
      <div style={{ maxWidth:"880px",margin:"0 auto 64px",padding:"0 24px" }}>
        <div style={{ background:`linear-gradient(135deg,${RED}18,#0a0808 60%,${RED}0c)`,border:`1px solid ${RED}44`,borderRadius:"14px",padding:"28px 36px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"24px",boxShadow:`0 0 40px ${RED}14` }}>
          <div style={{ flex:1,minWidth:"220px" }}>
            ...
            <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:"clamp(22px,3.5vw,34px)",color:WHITE,letterSpacing:"1px",lineHeight:1.1 }}>
              ...
              <span style={{ color:REDB,textShadow:`0 0 12px ${RED}77` }}>competitor</span>{" "}books.
            </div>
          </div>
          <div style={{ textAlign:"center",padding:"0 8px" }}>
            <div className="red-loss" style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:"clamp(44px,7vw,72px)",color:WHITE,lineHeight:1,textShadow:`0 0 20px ${REDB}, 0 0 40px ${RED}88` }}>73%</div>
      ...
      <section style={{ maxWidth:"980px",margin:"0 auto",padding:"0 24px 80px" }}>
        <div style={{ textAlign:"center",marginBottom:"40px" }}>
          ...
          <h2 style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:"clamp(32px,5vw,58px)",color:WHITE,margin:"0 0 12px",letterSpacing:"2px",textShadow:"0 0 30px rgba(255,255,255,0.06)" }}>
            Watch Your AI Employee Work
          </h2>
          <p style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"15px",color:MID,maxWidth:"480px",margin:"0 auto",lineHeight:1.75 }}>
        ...
        <div style={{ display:"flex",gap:"4px",marginBottom:"20px",background:"#0d0d0d",border:"1px solid #222",borderRadius:"10px",padding:"4px" }}>
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)}
              style={{ flex:1,padding:"10px 8px",borderRadius:"7px",fontFamily:"'DM Sans',sans-serif",fontSize:"12px",fontWeight:"600",cursor:"pointer",border:"none",transition:"all 0.2s",background:tab===t.id?"#1c1c1c":"transparent",color:tab===t.id?WHITE:DIM,boxShadow:tab===t.id?"0 1px 8px rgba(0,0,0,0.7)":"none",whiteSpace:"nowrap" }}>
              {t.label}
            </button>
          ))}
        </div>
      ...
      {/* DIVIDER — demo → how it works */}
      <div style={{ padding:"0 0 48px" }}>
        <Divider color={GOLD} dim="14"/>
      </div>

      {/* HOW IT WORKS */}
      <section style={{ background:"#0a0a0a",borderTop:"1px solid #1c1c1c",borderBottom:"1px solid #1c1c1c",padding:"72px 24px" }}>
        <div style={{ maxWidth:"880px",margin:"0 auto",textAlign:"center" }}>
          <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"9px",color:DIM,letterSpacing:"3px",marginBottom:"12px" }}>HOW IT WORKS</div>
          <h2 style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:"clamp(32px,5vw,52px)",color:WHITE,margin:"0 0 10px",letterSpacing:"2px",textShadow:"0 0 30px rgba(255,255,255,0.05)" }}>One System. Five AI Agents.</h2>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:"12px" }}>
            {[
              ...
              <div key={a.n}
                style={{ background:"#0d0d0d",border:"1px solid #222",borderRadius:"12px",padding:"24px 14px",textAlign:"center",transition:"all 0.25s",cursor:"default" }}
                ...
                <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"8px",color:DIM,marginBottom:"4px",letterSpacing:"1px" }}>AGENT {a.n}</div>
                <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:"20px",color:GOLD,marginBottom:"6px",letterSpacing:"2px",textShadow:`0 0 10px ${GOLD}44` }}>{a.name}</div>
                <div style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"12px",color:MID,lineHeight:1.5 }}>{a.desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding:"80px 24px 96px",textAlign:"center" }}>
        <div style={{ maxWidth:"580px",margin:"0 auto" }}>
          <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"9px",color:DIM,letterSpacing:"3px",marginBottom:"18px" }}>STOP LEAVING MONEY ON THE TABLE</div>
          ...
          <p style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"16px",color:MID,margin:"0 0 10px",lineHeight:1.75 }}>
            Book a free 30-minute demo. We'll show you exactly how Mano works — and what it's costing you every day you wait.
          </p>
          <p style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"14px",color:REDB,margin:"0 0 36px",fontStyle:"italic",opacity:0.85 }}>
          ...
          <div style={{ marginTop:"18px",fontFamily:"'Space Mono',monospace",fontSize:"10px",color:"#555",letterSpacing:"1px" }}>Free 30-min walkthrough. No pressure.</div>
        </div>
      </section>

      {/* FOOTER */}
      <footer style={{ borderTop:"1px solid #1e1e1e",padding:"28px 28px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:"8px" }}>
        <div>
          <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:"16px",color:MID,letterSpacing:"3px" }}>MONKEE BIZZ AI</div>
          <div style={{ fontFamily:"'Space Mono',monospace",fontSize:"8px",color:DIM,letterSpacing:"2px",marginTop:"2px" }}>POWERED BY MANOLOGICS</div>
        </div>
