...
// ─── COLOR SYSTEM ─────────────────────────────────────────────────────────────
const GOLD  = "#f5c518";
const GOLDD = "#c9a000";   // deeper gold for gradients
const RED   = "#cc2020";   // slightly deeper / more aggressive red
const REDB  = "#e03030";   // bright red for accents
const WHITE = "#ffffff";
const GREEN = "#00ff88";   // AI/system status only
...
function PreQualModal({ onClose }) {
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.97)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:"20px" }}>
      <div style={{ background:"#0d0d0d",border:`1px solid ${GOLD}22`,borderRadius:"16px",padding:"36px 32px",maxWidth:"420px",width:"100%",position:"relative",boxShadow:`0 0 60px ${GOLD}10` }}>
        ...
        {step===1&&<>
          ...
          <button onClick={()=>biz&&setStep(2)} className={biz?"gold-glow-btn":""} style={{ width:"100%",padding:"14px",borderRadius:"8px",fontFamily:"'Space Mono', monospace",fontSize:"12px",fontWeight:"700",border:"none",cursor:biz?"pointer":"not-allowed",background:biz?`linear-gradient(135deg,${GOLD},${GOLDD})`:"#1a1a1a",color:biz?"#000":"#333",letterSpacing:"1px" }}>NEXT →</button>
        </>}
        {step===2&&<>
          ...
          <div style={{ display:"flex",gap:"10px" }}>
            ...
            <button onClick={()=>{if(vol){window.open(CALENDLY_URL,"_blank");onClose();}}} className={vol?"gold-glow-btn":""} style={{ flex:1,padding:"14px",borderRadius:"8px",fontFamily:"'Space Mono', monospace",fontSize:"11px",fontWeight:"700",border:"none",cursor:vol?"pointer":"not-allowed",background:vol?`linear-gradient(135deg,${GOLD},${GOLDD})`:"#1a1a1a",color:vol?"#000":"#333",letterSpacing:"1px" }}>BOOK MY DEMO →</button>
          </div>
        </>}
...
export default function Demo() {
  ...
  function CTASecondary({ style={} }) {
    return (
      <button
        onClick={()=>setShowModal(true)}
        style={{ background:"transparent",color:"#555",border:"1px solid #1e1e1e",padding:"16px 28px",borderRadius:"9px",fontFamily:"'DM Sans', sans-serif",fontSize:"13px",fontWeight:"600",cursor:"pointer",transition:"all 0.2s",...style }}
