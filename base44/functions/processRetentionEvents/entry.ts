...
const TEMPLATES = {
  satisfaction_check: {
    ...
    body: (name, biz, reviewLink, referralOffer, upsellLink) => `<div style="font-family:Arial,sans-serif;max-width:600px;background:#0a0a0a;color:#fff;padding:32px;border-radius:16px">
      <p>Hi ${name},</p>
  ...
  review_request: {
    ...
    body: (name, biz, reviewLink) => `<div style="font-family:Arial,sans-serif;max-width:600px;background:#0a0a0a;color:#fff;padding:32px;border-radius:16px">
      <p>Hi ${name},</p>
  ...
  referral_ask: {
    ...
    body: (name, biz, reviewLink, referralOffer) => `<div style="font-family:Arial,sans-serif;max-width:600px;background:#0a0a0a;color:#fff;padding:32px;border-radius:16px">
      <p>Hi ${name},</p>
...
Deno.serve(async (req) => {
  try {
    ...
    for (const evt of due) {
      ...
      // TASK 4 — Review request: check review_received first
      ...
      try {
        await S.integrations.Core.SendEmail({
          ...
          body: tmpl.body(lead.name, businessName, reviewLink, referralOffer, upsellLink)
        });
